 ...
 * Version: 1.1.0
 * Description: Lógica da animação orbital, compatível com Elementor e Gutenberg.
 */
(function ($) {
	'use strict';
	 ...
	 * Todo o código da animação está encapsulado aqui.
	 */
	var runOrbitalAnimation = function (wrapper) {
		// Previne reinicialização se já estiver a correr
		...
		// O resto do seu código de animação (início)
        // Esta parte é idêntica à das versões anteriores
		let config;
		try {
		...
		function setupCanvas() {
			...
			meteors.forEach(m => {
				m.orbitRadius = mainOrbitRadius + outerOrbitSpacing * (m.orbitIndex + 1);
			});
		...
		// Usar um ResizeObserver é mais eficiente para redimensionamentos.
		new ResizeObserver(fullResetAndStart).observe(wrapper);
        // O resto do seu código de animação (fim)
	};

	/**
	 * Função de inicialização que decide quando executar a animação.
	 * Usa IntersectionObserver para performance, iniciando a animação apenas quando visível.
	 */
	...
	// 1. Gancho para o Elementor: Executa quando um widget está pronto.
	...
	// 2. Fallback para não-Elementor (Gutenberg, Editor Clássico, etc.)
	...
	$(function () {
		...
		containers.forEach(function (container) {
			// Verifica se o elemento não está dentro de um widget Elementor para não duplicar a inicialização.
